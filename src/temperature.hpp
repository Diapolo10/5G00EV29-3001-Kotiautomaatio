...
namespace maths {
    ...
    static const int resistor{10000};              // Ohm
    static const float zero_celsius{273.15};  // Kelvin

    double calc_resistance(double adc_voltage, bool ntc_not_pot = true) {
        double res = ntc_not_pot ? 10000 : 9300;
        return res / (4095 / adc_voltage - 1);
    ...
    double calc_temp(double adc_voltage, bool ntc_not_pot = true) {
        return temperature_c(calc_resistance(adc_voltage, ntc_not_pot));
    }
}
